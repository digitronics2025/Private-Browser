...
| `windows-installer` | windows, 25 min, needs `verify` | Selects the next stable version on `main`, writes the bundled update bootstrap (see **Bundled Bootstrap**), runs `npm run dist`, then writes the checksum and `VERSION.txt`; smoke-installs the VSIX into an isolated VS Code profile; uploads `private-browser-windows` and the separate `private-browser-bridge-vsix` artifact (`if-no-files-found: error`, 30-day retention) |
...
- `npm run dist` is `npm run build && electron-builder --win nsis --x64 --publish
  never`, followed by the versioned VSIX staging step. `--publish never` is
  deliberate: electron-builder uploads nothing, and the Cloudflare job is the
  only installer publisher.
- `release/private-browser-bridge-0.4.0.vsix` is generated by `@vscode/vsce`,
  validated for its manifest and forbidden contents, installed into a fresh
  `--user-data-dir`/`--extensions-dir` profile in Windows CI, and uploaded as a
  private Actions artifact. It is never submitted to the Marketplace. The same
