...
- [x] T5. Committed path-scoped and pushed — done when: `git status` shows none of this work uncommitted and the push succeeded — check: `git log origin/main..HEAD --oneline` — pushed `a02c352..0b1d493` after the operator asked for it. The remote had moved four commits (a redesigned installer page and a stable tokenized download route from another session); merged rather than rebased so their commits kept their identities, resolving conflicts in `cloudflare/tests/worker.test.ts` and `docs/systems/release-and-updates.md` by keeping both sides. Gate green on the merged tree; tree clean.
- [x] T6. Confirmed live where the push deploys — done when: the release service is observed on the deployed target after the push, or the step records why the push must not publish yet — check: `manual: what was called on the live target and what it showed` — confirmed live. `Deploy download service` succeeded (and now runs the whole gate first, F-10). `GET /health` on the live Worker returns 200 `{"status":"ok","database":"ok","releaseReady":true}` and unauthenticated `/update.json` returns 404, so the fail-closed rule holds in production — this also closes the one COULD NOT CHECK the audit had to leave open. The publish job then **failed exactly as designed**: `Release registration failed (409): {"error":"release_version_not_bumped","detail":"active release is 0.3.1; bump the version before publishing"}`.
- [x] T7. A claim registered for this change — done when: this repo's claims record names what should now be true with a probe reading downstream of the diff and a deadline, or the step says "no observable outcome" with the reason — check: `manual: name the claim and its deadline, or say why not` — claim registered here, with its first reading taken. **Claim:** *a push that does not raise the version can no longer mint a release, and a documentation-only push cannot publish at all.* **Probe** (reads the database the pipeline writes, downstream of the workflow and Worker changes rather than the row either wrote): `npx wrangler d1 execute private-browser-releases --remote --command "SELECT id, version, build_number, substr(commit_sha,1,7), is_active FROM releases ORDER BY build_number DESC LIMIT 3"`.

**First reading, 2026-09-10, and it is the evidence for F-04.** Production held `stable-0.3.1-8` (commit 63e5ee8), `-9` (a63daad) and `-10` (a02c352) — three different installers under one version number, none of which any installed client would ever have been offered, and build 10 came from a documentation-only commit. That is the finding happening, three times, in the live system. Build 11 was then refused. **Answer date: the next push that changes code.** The active row must still read `stable-0.3.1-10` unless the version was deliberately raised.
...
- 2026-09-10 — T5 — the push was authorised by the operator on a second ask. `origin/main` had moved four commits meanwhile. **Merged, not rebased**: two of the local commits belonged to the other session and a rebase would have rewritten their identities. The test-file conflict was resolved by keeping their two new stable-page tests *and* the point of mine — the page must carry this release's own version, checksum and commit rather than a hard-coded template string, which is the F-09 assertion that could not fail. Re-stamped four system docs to the merge commit.
- 2026-09-10 — T6/T7 — the live probe turned F-04 from an argument into a record: builds 8, 9 and 10 all sit in production at version 0.3.1, and build 10 came from a docs-only commit — the exact failure the finding described, three times over, before the fix was live. Build 11 was refused with the intended 409. The health probe also closed the audit's only COULD NOT CHECK.
