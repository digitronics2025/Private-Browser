...
- 2026-09-24 21:05 — T5/T6 resolved after the owner replied "merge and push": `feat/search-home-settings` fast-forwarded into `main` as `746f186` and pushed; the worktree (junctions unlinked first) and the branch were removed. CI run 36049499069 then failed on the Turnstile Electron test (`ERR_ABORTED`), so nothing was published. Root cause: a `showActiveTab` double-load race (see the 0.7.0 plan Ledger, step 9), fixed in `3c60646`, proven with `--repeat-each 5` (at least 4/5 failing without the fix, 20/20 with it), full `npm run check` green, pushed. CI run 36050835839 passed and published **0.6.4 build 113** (`Private-Browser-0.6.4-Setup.exe`, sha256 `d2a7bc14…75c4`); its `verify-live-release.mjs` step downloaded the live installer end to end with a matching hash. Not yet checked: the new settings on an installed copy (see follow-ups)
