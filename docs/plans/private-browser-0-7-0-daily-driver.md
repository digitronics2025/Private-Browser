...
- [ ] 9. S1 Make the Turnstile Electron test order-independent — done when: the spec passes run alone and run after the rest of its file, with the root cause in the Ledger — check: `npx playwright test --config playwright.electron.config.ts tests/electron/account-spaces.spec.ts`
