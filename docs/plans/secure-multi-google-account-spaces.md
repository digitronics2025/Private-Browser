...
- [x] 10. Implement cryptographic backup primitives and Drive app-data boundary — done when: AES-256-GCM versioned envelopes, recovery-code verification, exclusions, tamper rejection, ETag conflict decisions and disabled-on-missing-key behavior pass tests, with Google receiving ciphertext only — check: `npm test -- --run tests/account-backup.test.ts`
- [x] 11. Harden and extend typed IPC — done when: new lifecycle/OAuth/service/permission/progress channels use schema validation, trusted-main-frame sender checks, channel plus global rate limits, UUID/membership/body caps, cancellable operations, and no `any` callback or generic Google URL fetch — check: `npm test -- --run tests/ipc.test.ts tests/ipc-account-spaces.test.ts`
- [x] 12. Extend AI consent and privacy logging — done when: AI capabilities bind account, tab, service and revision; Google data defaults local; cloud use keeps sanitized one-request approval; Banking and secret-content exclusions remain enforced — check: `npm test -- --run tests/ai-consent.test.ts tests/ai-account-spaces.test.ts`
...
- 2026-09-12 12:02 +01:00 — step 10 — added AES-256-GCM authenticated envelopes with versioned AAD, random 256-bit recovery material verified before safeStorage wrapping, allowlisted backup payloads, opt-in tab/history inclusion, Drive appDataFolder ciphertext-only transport, bounded downloads, ETag conflict detection, explicit merge/overwrite and cross-device recovery; six backup tests and both type targets passed.
- 2026-09-12 12:08 +01:00 — step 11 — replaced the `any` IPC callback surface with typed tuples, added global plus sensitive per-channel budgets, strict Account Space/UUID/workspace/module/browser/permission/body/calendar/mail/Drive validators, retained trusted-main-frame checks, exposed only narrow service/backup commands, and added account-bound cancellation plus sanitized progress events; seven IPC tests and both type targets passed.
- 2026-09-12 12:10 +01:00 — step 12 — bound every AI preview and single-use approval to its opaque Account Space, exact tab, full source URL, classified service and SHA-256 source revision; retained Banking/payment denial and sanitized origin-only cloud context, categorically blocked mailbox content from model input, and reduced provider requests to bounded ten-second calls; six targeted consent tests and both type targets passed.
