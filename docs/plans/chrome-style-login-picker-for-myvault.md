...
- 2026-09-24 22:30 — release — operator said "send it live". origin/main had moved to 746f186 (search/home settings, already pushed by another session, no vault sources touched); rebased feat/fill-picker onto it cleanly and re-pointed vault.md `verified_at` at the rebased commit. Rerunning the full gate on the combined tree before pushing to main.
- 2026-09-24 22:45 — release — gate on the rebased tree exit 0 (311 + 7 + 6 + 24 unit, 22 browser e2e, 15 Electron incl. the four fill-picker specs, Worker dry-run); origin/main still 746f186, so pushed feat/fill-picker to main as a fast-forward. Supersedes T5/T6's "not pushed": the push starts the release pipeline.
