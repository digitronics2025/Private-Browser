...
export function installLocalBridgePreview(): void {
  ...
  const snapshot: BrowserSnapshot = { settings: { ...DEFAULT_BROWSER_SETTINGS }, workspaces, activeWorkspaceId: 'development', activeAccountSpaceId: developmentAccount.id, activeTabId: 'development', tabs, bookmarks: [], history: [], downloads: [], privacyLog: [], trackerBlocking: true, bookmarkBarVisible: true, accountSpaces: accounts, accountHealth: accounts.map((account) => ({ accountSpaceId: account.id, status: 'disconnected', checkedAt: '2026-09-12T10:00:00Z' })), googleConfiguration: { configured: false }, externalBrowsers: [], ui: { theme: 'system', bookmarkBarMode: 'always', sidePanelOpen: true, sidePanelWidth: 400, sidePanelTool: 'developer', newTabBackground: 'plain' }, windowState: { maximized: false, fullscreen: false, darkMode: true }, shortcutsByAccountSpace: {}, bookmarkIcons: {}, canReopenClosedTab: false, sideApps: [] };
