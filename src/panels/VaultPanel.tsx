...
export function VaultPanel({ workspaceId, activeOrigin, onOpenFull, onToast }: { workspaceId: WorkspaceId; activeOrigin?: string; onOpenFull?: () => void; onToast: (message: string, kind?: 'ok' | 'error') => void }) {
  ...
  return (
    <div className="side-panel">
      ...
      {status.lifecycle === 'unlocked' && status.platformUnlock === 'available' && <button className="text-button" onClick={() => void hello(() => window.privateBrowser.enableVaultHello(), { message: 'Windows Hello can now unlock MyVault', when: 'enrolled' })}><Fingerprint size={14} /> Turn on Windows Hello unlock</button>}
      {status.lifecycle === 'unlocked' && status.platformUnlock === 'enrolled' && <button className="text-button" onClick={() => void hello(() => window.privateBrowser.disableVaultHello(), { message: 'Windows Hello unlock turned off', when: 'available' })}><Fingerprint size={14} /> Turn off Windows Hello unlock</button>}
