...
function html(kind: SecureDialogKind, nonce: string, origin?: string): string {
  const title = kind === 'unlock' ? 'Unlock MyVault' : kind === 'pair' ? 'Connect MyVault' : kind === 'confirm-delete' ? 'Confirm deletion' : kind === 'confirm-fill' ? 'Confirm Banking fill' : kind === 'confirm-capture' ? 'Confirm captured login' : kind === 'confirm-replace' ? 'Resolve changed login' : kind === 'confirm-cleanup' ? 'Remove legacy vault' : 'Save in MyVault';
  const action = kind === 'confirm-delete' ? 'Delete login' : kind === 'confirm-fill' ? 'Fill this origin' : kind === 'confirm-capture' ? 'Encrypt and save' : kind === 'confirm-replace' ? 'Replace login' : kind === 'confirm-cleanup' ? 'Delete old ciphertext' : kind === 'unlock' ? 'Unlock' : kind === 'pair' ? 'Connect securely' : 'Encrypt and save';
